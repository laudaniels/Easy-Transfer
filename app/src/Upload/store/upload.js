...
// Folder-drop (drag-drop's file.fullPath, e.g. "/Folder/Sub/file.txt") and
// folder-picker (webkitdirectory's file.webkitRelativePath, e.g. "Folder/Sub/file.txt")
// both carry the original path; plain file picks/drops have neither.
function splitRelativePath(file) {
  const raw = file.fullPath || file.webkitRelativePath || '';
  if (!raw) return { relativePath: '', name: file.name };
  const segments = raw.replace(/^\/+/, '').split('/').filter(Boolean);
  const name = segments.pop() || file.name;
  return { relativePath: segments.join('/'), name };
}
  ...
  actions: {
    addFiles({ commit, state, rootState }, files) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const { relativePath, name } = splitRelativePath(files[i]);
        commit('ADD_FILE', {
          _File: files[i],
          name,
          relativePath,
