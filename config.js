...
const config = {
  ...
  "trustProxy": false, // set to 'loopback' if running behind an nginx reverse proxy on the same host
...
//    "259200": "3 Days",
  ...
  // "From:" address used by the file-downloaded-email plugin's notification emails
  ...
  "defaultLanguage": "en",
  // Limit upload size
  "maxFileSize": Math.pow(2, 30) * 5, // 5GB, matches nginx's client_max_body_size
  "maxBucketSize": Math.pow(2, 30) * 5, // 5GB per share (one large file, or many small ones)
  "plugins": ['file-downloaded-webhook', 'file-uploaded-webhook', 'file-downloaded-email'],
