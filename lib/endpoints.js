...
function safeArchiveEntryPath(relativePath, name, fallback) {
  const safeName = utils.toSafeBasename(name, fallback);
  // defense in depth: re-validate even though this was already checked at upload time
  const safeRel = utils.isSafeRelativePath(relativePath) ? relativePath : '';
  return safeRel ? `${ safeRel }/${ safeName }` : safeName;
}
...
// throttle admin-password guessing: 10 attempts / 15 min per IP
const adminLoginLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 10,
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: true, // only count wrong-password attempts, not normal refreshes
  message: 'Too many attempts, please try again later.',
});

// throttle bucket download-password guessing: 10 attempts / 15 min per IP
const downloadPasswordLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 10,
  standardHeaders: true,
  legacyHeaders: false,
...
// throttle upload-password guessing: 10 attempts / 15 min per IP
// shared across /config.json and /files since both gate the same config.uploadPass secret
// /files has many legitimate non-password 4xx outcomes (invalid bucket id, locked bucket,
// bad tus metadata, etc.) so only count the password check's own 401/403, not those
const uploadPasswordLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 10,
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: true, // activates the requestWasSuccessful check below
  requestWasSuccessful: (req, res) => res.statusCode !== 401 && res.statusCode !== 403,
  message: 'Too many attempts, please try again later.',
});

// throttle new-upload creation (tus POST) to curb storage/inode exhaustion via mass bucket spam.
// PATCH chunk requests for an already-created upload are skipped: a single legitimate large
// upload is ~1000 PATCH chunks (5GB max file / 5MB chunks) and must not be throttled.
// max is set high (2500) since a single legitimate folder upload can itself be ~1000-2000
// files, each needing its own POST create - this must stay well above any realistic bulk upload.
// Counts successes too (not just failures) - the risk is volume of created uploads, not guessing.
const newUploadLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 2500,
  standardHeaders: true,
  legacyHeaders: false,
  skip: (req) => req.method !== 'POST',
  message: 'Too many new uploads, please try again later.',
});
...
// Config
app.get(`${ config.baseUrl }config.json`, uploadPasswordLimiter, (req, res) => {
...
app.get(`${ config.baseUrl }admin/data.json`, adminLoginLimiter, async (req, res, next) => {
  if (!config.adminPass) return next();
  ...
  if (!(await verifyPassword(config.adminPass, req.get('x-passwd')))) {
    setTimeout(() => res.status(403).send('Forbidden'), bfTimeout);
    return;
  }
...
app.get(`${ config.baseUrl }:sid`, downloadPasswordLimiter, async (req, res, next) => {
...
app.get(`${ config.baseUrl }files/:fid`, async (req, res, next) => {
  ...
  if (req.params.fid.match(/^[a-z0-9+]+\.(tar\.gz|zip)$/)) {
    ...
    if(format === 'zip') {
      ...
      const uniqueName = (rawName, fallback, relativePath) => {
        const base = safeArchiveEntryPath(relativePath, rawName, fallback);
      ...
      for (const info of bucket) {
        await new Promise((resolve, reject) => {
          ...
          archive.append(stream, { name: uniqueName(info.metadata.name, info.key, info.metadata.relativePath) });
        });
      }
      ...
      for (const info of bucket) {
        await new Promise((resolve, reject) => {
          ...
          const entry = pack.entry({ name: uniqueName(info.metadata.name, info.key, info.metadata.relativePath), size: info.size });
  ...
  async function(req, res, next) {
    ...
    if (req.method === 'POST') {
      ...
      try {
        ...
        // The actual filename is always a flat basename - no directories, no absolute
        // paths, no traversal, no control chars.
        if (!utils.isSafeBasename(meta.name)) {
          return res.status(400).end('Invalid file name');
        }

        // Optional folder path (e.g. from a dragged/selected folder), used only to
        // reconstruct structure in zip/tar downloads - validated the same way, per segment.
        meta.relativePath = typeof meta.relativePath === 'string' ? meta.relativePath : '';
        if (!utils.isSafeRelativePath(meta.relativePath)) {
          return res.status(400).end('Invalid folder path');
        }
